...
<?php
$config['smtp_username'] = 'mishamoroz59@gmail.ru';  //Смените на адрес своего почтового ящика.
$config['smtp_port'] = '465'; // Порт работы.
$config['smtp_host'] =  'ssl://smtp.gmail.com';  //сервер для отправки почты
$config['smtp_password'] = 'Mishamoroz22042014';  //Измените пароль
...
<body>
    <div class="otvet">
        ...
        if (isset($_POST['submit'])) {
            ...
            if (smtpmail($to, $message, "Вам письмо", "From: php server")) {
                echo "Your letter has been sent<br>";
            } else {
                echo "Having problems sending<br>";
            }
        }
        ?>
    </div>
    <div class="arrayForm">
        ...
        <form method="post">
            <div class="form-row">
                <label for="name">Your name</label><input type="text" id="name" name="name" required /><br>
                <label for="email">E-mail</label><input type="email" id="email" name="email" required /><br><br>
                <textarea placeholder="Enter message" name="message" required></textarea><br>
